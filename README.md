...
将 .env.example 重命名为 .env，然后打开填入你的密钥：
...
USE_API = "agnes" 改为 "deepseek" 可切换

快速模式

前端勾选 “⚡ 快速模式”，自动跳过画面描述、关闭反思和工具调用，速度提升 3~5 倍。

自定义提示词

在配置面板输入额外指令，例如：请重点分析案例，用通俗语言解释
...
首次加载约 1~2 分钟，请耐心等待，并检查模型文件夹是否完整

Q5: 历史记录里为什么有两条相同的笔记？

访问 http://127.0.0.1:8000/cleanup 可自动清理无效记录
